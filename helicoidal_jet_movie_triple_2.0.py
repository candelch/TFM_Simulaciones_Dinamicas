...
def save_scientific_pdf(im_arr, beamparams, filename, title, vmax=None, cmap='viridis'):
    ...
    if vmax is None:
        vmax = np.max(im_arr_mJy) * 0.9 if np.max(im_arr_mJy) > 0 else 1.0
    else:
        vmax = vmax * 1000.0 
        
    im = ax.imshow(im_arr_mJy, origin='lower', cmap=cmap, extent=extent_mas, vmin=0, vmax=vmax)
